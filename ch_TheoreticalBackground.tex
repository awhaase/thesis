...
\end{align}
The $z$-component of the complex wave vector in each layer $j$ is denoted with $k_z^{(j)}$. It is calculated according to the incoming wave vector in vacuum $k^\text{vac} = 2\pi/\lambda$ with Snell's law
\begin{align}
k_z^{(j)} &= \sqrt{\Big(n^{(j)} k^{\text{vac}}\Big)^2 - k_x^{\text{vac}}}
\end{align}
where $k^\text{vac}_x = \cos(\alpha_i) k^\text{vac}$ with the angle of incidence $\alpha_i$ defined from the surface normal (cf.~Fig.~\ref{fig:geometry}) and $n$ is the complex index of refraction of layer $j$. The transmitted and reflected fields can then be calculated based on the following matrix equation with the boundary conditions of a known incoming electric field $E_0$ and the vanishing reflected field inside the infinite substrate
\begin{align}
\begin{pmatrix}
E_0 \\
E_R
