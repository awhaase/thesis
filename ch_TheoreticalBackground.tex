...
The EUV and X-ray reflectivities were calculated based on the well-established matrix algorithm \cite{born_principles_1965,mikulik_x-ray_1997}. The roughness and interdiffusion are taken into account by using modified Fresnel coefficients including a N\'{e}vot-Croce factor using $\sigma$ to describe the disturbances at the interfaces \cite{croce_p._etude_1976}. We assume the interdiffusion and roughness profile to be of error-function like shape, which leads to the modified Fresnel coefficients
\begin{align}
        r^{(j)} &= r_{id}^{(j)} \exp(-2 k_z^{(j)} k_z^{(j+1)} 
\sigma^2)\text{,} \nonumber \\
        t^{(j)} &= t_{id}^{(j)} \exp((k_z^{(j)} - k_z^{(j+1)})^2 \sigma^2/2) 
\text{,} \label{eqn:mod_fresnel}
\end{align}
where $r_{id}^{(j)}$ and $t_{id}^{(j)}$ are the unmodified Fresnel coefficients for an ideal multilayer system at each interface $j$, defined as
\begin{align}
        r_{id}^{(j)} &= \frac{k_z^{(j)} - k_z^{(j+1)}}{k_z^{(j)} + 
k_z^{(j+1)}}\text{,} \nonumber \\
        t_{id}^{(j)} &= \frac{2 k_z^{(j)}}{k_z^{(j)} + k_z^{(j+1)}}\text{.} \label{eqn:id_fresnel}
\end{align}
The $z$-component of the complex wave vector in each layer $j$ is denoted with $k_z^{(j)}$. It is calculated according to the incoming wave vector in vacuum $\gls{k_0} = 2\pi/\lambda$ with Snell's law
\begin{align}
k_z^{(j)} &= \sqrt{\Big(n^{(j)} \gls{k_0}\Big)^2 - k_x^2}
\end{align}
where $k_x = \cos(\alpha_i) \gls{k_0}$ with the angle of incidence $\alpha_i$ defined from the surface normal (cf.~Fig.~\ref{ch_theo:fig_scattering_process}) and $n^{(j)}$ is the complex index of refraction of layer $j$. The transmitted and reflected fields can then be calculated based on the following matrix equation with the boundary conditions of a known incoming electric field $E_0$ and the vanishing reflected field inside the infinite substrate
\begin{align}
\begin{pmatrix}
E_0 \\
E_R
...
\end{align} 
The propagation matrix is defined as \cite{mikulik_x-ray_1997}
\begin{align}
M_j = \frac{1}{t^{(j)}}
\begin{pmatrix}
1 & r^{(j)} \\
