...
With the two additional measurements described above, the total of four data sets (\gls{euv}, \gls{xrr}, \gls{reuv} and \gls{xrf}) exists for the Cr/Sc multilayer sample to reconstruct the parameters of the gradual interface model. In analogy to the combined analysis conducted for the Mo/Si/C systems in Sec.~\ref{ch_spec:sec_MoSi_euv_xrr_combined}, we define the minimization functional for the combined analysis of all the datasets as
\begin{align}
...
Similar, as for the other two sample systems treated in this chapter, the parameters or the model, here the gradual interface model with the parameters and their limits listed in table~\ref{ch_spec:tbl_CrSc_gradual_parametrization}, were first obtained using the \gls{pso} method to find a solution reproducing the experimental results. Secondly, following the maximum likelihood approach employing the \gls{mcmc} method as detailed in Sec.~\ref{ch_spec:sec_maximum_likelihood}, starting from this the uniqueness and confidence intervals for each parameter were obtained.
