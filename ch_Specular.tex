...
Firstly, similar as for the other two sample systems treated in this chapter, the parameters of the model, here the gradual interface model with the parameters and their limits listed in table~\ref{ch_spec:tbl_CrSc_gradual_parametrization}, were first obtained using the \gls{pso} method to find a solution reproducing the experimental results. Secondly, following the maximum likelihood approach employing the \gls{mcmc} method as detailed in Sec.~\ref{ch_spec:sec_maximum_likelihood}, starting from this the uniqueness and confidence intervals for each parameter were obtained.

Through the minimization of the combined $\chi^2$ functional in Eq.~\eqref{ch_spec:eqn_CrSc_total_chi_2} via the \gls{pso} method, the best model parameters were obtained and are listed in table~\ref{ch_spec:tbl_CrSc_best_model}
\begin{table}[htbp]
\centering
\caption{Optimized model parameters obtained by \gls{pso} analysis}
\label{ch_spec:tbl_CrSc_best_model}
\begin{tabular}{@{}ll@{}}
\toprule
Parameter &  Combined \\ \midrule
$D$ / nm& $1.5737$  \\ 
$\Gamma_{Sc}$ & $0.48$  \\ 
$s_d$ / nm& $1.34$  \\ 
$\Gamma_\sigma$ & $0.16$  \\ 
...
\end{tabular}
\end{table}
The resulting theoretical curves are shown in comparison with the data from Fig.~\ref{ch_spec:fig_CrSc_all_data} including the respective uncertainties in Fig.~\ref{ch_spec:fig_combined_fit_result}.
\begin{figure*}[htbp]
  \centering
...
\end{figure*}
The data and the best model show excellent agreement indicating that the gradual interface model as defined above indeed provides a very good representation of physical reality with respect to the experiments conducted here.

\subsubsection{Confidence Intervals an Evaluation of the Experimental Methods}
In order to assess the information content within each of the experimental methods, the \gls{pso} and \gls{mcmc} analysis was conducted for each experiment individually and for the 
combination of all experiments. In the case of the \gls{xrr} measurements, only the 
...
based exclusively on the \gls{xrr} data. The results were added as fixed surface 
layers to the model for all theoretical calculations of all experiments (cf.~model illustration in Fig.~\ref{ch_spec:fig_Cr_Sc_model}). The 
analysis of the \gls{xrf} experiment was conducted based on the fluorescence data 
at an excitation photon energy of \ev{6250} for the Sc-K and Cr-K lines by spectral 
