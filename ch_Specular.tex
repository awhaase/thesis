...
\caption[Theoretical EUV reflectance curve for the Mo/B$_4$C/Si/C sample.]{Theoretical reflectance curve for the Mo/B$_4$C/Si/C sample based on the optimal model parameters obtained from the particle swarm optimization.}
...
\begin{table*}
\centering
\caption[Parametrization of the Mo/Si/C multilayer samples.]{Parametrization of the Mo/Si/C multilayer samples with varying molybdenum layer thicknesses.}
...
\caption[Correlation of silicon and carbon layer thickness parameters in the model.]{Two-dimensional likelihood distribution indicating the correlation of silicon and carbon layer thickness. The distribution was obtained by marginalizing over all remaining parameters of the model. The blue lines indicate the fit obtained through the two-step \gls{mcmc} optimization procedure (see main text).}
...
\caption[Experimental EUV reflectivity data in comparison with the theoretical curves for an unpolished Mo/Si/C sample.]{Experimental data in comparison with the theoretical curves calculated with the model parameters obtained from the combined analysis of \gls{euv} and \gls{xrr} data. The data shown here was measured on the unpolished sample with nominal molybdenum thickness of $d_\text{Mo}^\text{nom} = \nm{3.05}$.}
...
\begin{table}[htbp]
\centering
\caption[Nominal molybdenum layer thicknesses in the two Mo/Si/C sample sets.]{List of nominal molybdenum layer thicknesses in the two sample sets. Both sets were fabricated with a equidistant increase in thickness from \nm{1.70} to \nm{3.05} with 9 unpolished and 10 polished samples.}
...
To better illustrate the differences to the Mo/Si systems, an 
...
\begin{table}[htbp]
\centering
\caption[PSO fit results for the discrete layer Cr/Sc multilayer model.]{\Gls{pso} fit results for the discrete layer Cr/Sc multilayer model.}
...
\begin{figure}[htbp]
  ...
  \caption[Fitted EUV reflectance curves for the Cr/Sc sample.]{Fitted experimental EUV reflectance curves across the wavelength 
...
\begin{figure}[htbp]
  ...
  \caption{Comparison of the numerical uncertainty with the experimental uncertainty for the graded Cr/Sc model.}
...
\begin{figure}[htbp]
  ...
  \caption[EUV peak deformation for a constant drift in the Cr/Sc period thickness.]{\Gls{euv} peak deformation assuming a constant drift of $d_\text{drift} = \nm{0.005}$ across the total multilayer stack keeping the mean period thickness $D$ constant.}
...
\begin{figure}[htbp]
  ...
  \caption[Measured resonant EUV reflectivity curves across the Sc L2 and L3-edge.]{Measured resonant \gls{euv} reflectivity curves across the Sc L2 and L3-edge in logarithmic representation. At each equidistant photon energy point, an angular resolved reflectivity curve was recorded across the Bragg peak.}
...
\begin{table}[htbp]
\centering
\caption[Optimized model parameters of the capping layers in the Cr/Sc system.]{Optimized model parameters obtained by \gls{pso} analysis of the \gls{xrr} data to determine the structure of the capping layers.}
...
\begin{table*}
\centering
\caption[Optimized model parameters with confidence intervals for the Cr/Sc system.]{Optimized model parameters with confidence intervals derived from MCMC 
...
\begin{figure*}[htbp]
  ...
  \caption[Measured data and optimized theoretical curves the combined analysis of the Cr/Sc system.]{Measured reflectance and fluorescence yield curves in direct comparison with the calculated reflectance and intensity curves for the 
...
\includegraphics{img/CrSc_confidence_intervals_radar}
  \caption[Illustration of the confidence intervals for the Cr/Sc model parameters.]{Visual representation of the total confidence intervals for each of 
