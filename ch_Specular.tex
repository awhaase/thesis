...
\paragraph{Optimization functional and procedure} \label{ch_spec:sec_MoSi_euv_xrr_combined}
...
Due to the increased complexity of the model, the question arises how accurately any parameter of the model can be determined based on the available data and whether further analytical measurements can improve the result as this was clearly the result for the \gls{euv} and \gls{xrr} experiments shown above. For the particular case of the gradual interface model for periodic multilayer systems with sub-nanometer layer thicknesses, two additional analytic methods could possibly provide complementary information on the spacial distribution of the Cr and Sc. For that purpose, two additional experiments adding to the \gls{euv} and \gls{xrr} experiments were conducted on the sample system:
\begin{description}
 \item [\Gls{reuv}] The reflectivity of the sample was measured angular resolved across the first Bragg peak while tuning the wavelength in steps across the Sc L-edge. Optical constants by \cite{henke_x-ray_1993,aquila_measurements_2004}, Ch.~\ref{ch_exp}, Sec.~\ref{ch_exp:sec_multilayer_design}, Fig.~\ref{ch_exp:fig_crsc_contrast}
 \item [\Gls{xsw}] The fluorescence of the Cr K$_\alpha$ and Sc K$_\alpha$ lines were measured across the first Bragg peak as described in detail in Ch.~\ref{ch_theo}, Sec.~\ref{ch_theo:sec_xrf}
\end{description}


Following the maximum likelihood approach employing the \gls{mcmc} method as outlined in Sec.~\ref{ch_spec:sec_maximum_likelihood}, each parameter of the model was evaluated for each measurement.
